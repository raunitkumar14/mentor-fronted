...
function SimpleBarChart({ items, ariaLabel, emptyMessage }) {
  ...
  // Round the step up, then multiply — rounding each tick independently
  // (the old `Math.round((rawMax / Y_TICKS) * i)`) could make consecutive
  // labels uneven (e.g. max=10 gave 0, 3, 5, 8, 10: gaps of 3, 2, 3, 2).
  // Scaling against this rounded axisMax (>= rawMax, since it's a ceiling)
  // instead of rawMax keeps every bar within the plot area too.
  ...
  function yFor(value) {
    return AXIS_PAD_TOP + plotHeight - (value / axisMax) * plotHeight;
  }
    ...
    <section className="timeline-section">
      ...
      <div className="analytics-grid">
        ...
        <div className="analytics-subsection">
          ...
          {metrics && metrics.totalLeads > 0 && (
            <p className="analysis-total">
              Total Call Duration (Same Leads){" "}
              <span className="num">{formatDuration(metrics.totalCallDurationSec)}</span> ·{" "}
              Avg <span className="num">{formatDuration(metrics.avgCallDurationSec)}</span> / Lead
            </p>
          )}
          {viewMode === "telecaller" ? (
